...
export class DoctorService {
   ...
   */
  static async getAllDoctors(specialization?: string) {
    ...
    return doctors.map((doc) => ({
      ...
      user: {
        ...doc.user,
        name: formatDoctorName(doc.user.name),
      },
    }));
  }
  ...
  static async getAvailableSlots(doctorId: string, date: string) {
    ...
    const docFormattedName = formatDoctorName(doctor.user.name);

    // Check if doctor is on leave for this date
    const isOnLeave = doctor.leaves.some((l: { date: string }) => l.date === date);
    if (isOnLeave) {
      return {
    ...
    const bookedSlotSet = new Set(bookedAppointments.map((a: { timeSlot: string }) => a.timeSlot));

    // Fetch active unexpired slot holds
    const now = new Date();
    const activeSlotHolds = await prisma.slotHold.findMany({
      where: {
        doctorId,
        date,
        expiresAt: { gt: now },
    ...
    const heldSlotSet = new Set(activeSlotHolds.map((h: { timeSlot: string }) => h.timeSlot));

    // Map each slot with availability status
    const slotDetails = allSlots.map((slot) => {
      const isBooked = bookedSlotSet.has(slot);
      const holdInfo = activeSlotHolds.find((h: { timeSlot: string }) => h.timeSlot === slot);
   ...
   */
  static async addLeaveAndHandleConflicts(doctorId: string, date: string, reason?: string) {
    const jobsToEnqueue: Array<{ type: string; payload: any }> = [];
    ...
    const result = await prisma.$transaction(
      async (tx: any) => {
        ...
        return {
          ...
          affectedPatients: affectedAppointments.map((a: { id: string; patient: { name: string; email: string }; timeSlot: string }) => ({
            appointmentId: a.id,
    ...
    for (const job of jobsToEnqueue) {
      await QueueService.enqueueJob(job.type, job.payload);
    }
   ...
   */
  private static generateTimeSlots(startTime: string, endTime: string, durationMinutes: number): string[] {
    const slots: string[] = [];
