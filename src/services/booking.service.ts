...
export class BookingService {
   ...
   */
  static async holdSlot(patientId: string, doctorId: string, date: string, timeSlot: string) {
    const now = new Date();
    const expiresAt = new Date(now.getTime() + CONFIG.SLOT_HOLD_TTL_MINUTES * 60 * 1000);
  ...
  static async confirmBooking(
    ...
    const appointment = await prisma.$transaction(async (tx: any) => {
      ...
      const appt = await tx.appointment.create({
        data: {
          ...
          symptoms,
          status: 'BOOKED',
          preVisitSummary: 'Summary pending',
        },
        include: {
          patient: true,
          doctor: {
    ...
    try {
      preVisitSummary = await LLMService.generatePreVisitSummary(symptoms);
    ...
    try {
      gcalEventId = await GCalService.createAppointmentEvent({
    ...
    await QueueService.enqueueJob('EMAIL_CONFIRMATION', {
      ...
      subject: `Booking Confirmed with ${formattedDocName}`,
      textMessage: `Dear ${appointment.patient.name},\n\nYour appointment with ${formattedDocName} (${appointment.doctor.specialization}) on ${appointment.date} at ${appointment.timeSlot} has been successfully confirmed.\n\nReported Symptoms: ${symptoms}\nAI Symptom Assessment: ${preVisitSummary}\n\nThank you for choosing Sanjeevani!`,
