...
// Helper to format Doctor Name cleanly without duplicate "Dr. Dr." prefixes
...
// Auth State & Strict Page Routing
...
function showAuthView() {
  ...
  if (titleIndicator) titleIndicator.textContent = '';
}
...
function renderRolePage() {
  ...
  // Hide all role pages to enforce strict role isolation
  document.querySelectorAll('.role-page').forEach((el) => (el.style.display = 'none'));

  const titleIndicator = document.getElementById('page-title-indicator');

  // Strict page display: One cannot access another role's page
  if (currentUser.role === 'PATIENT') {
    document.getElementById('page-patient').style.display = 'block';
    if (titleIndicator) titleIndicator.textContent = 'Patient Dedicated Portal';
    loadDoctors();
    loadPatientAppointments();
  } else if (currentUser.role === 'DOCTOR') {
    document.getElementById('page-doctor').style.display = 'block';
    if (titleIndicator) titleIndicator.textContent = 'Doctor Dedicated Portal';
    loadDoctorSchedule();
  } else if (currentUser.role === 'ADMIN') {
    document.getElementById('page-admin').style.display = 'block';
    if (titleIndicator) titleIndicator.textContent = 'Admin Management Portal';
...
function renderUserNav() {
  ...
  userName.textContent = currentUser.role === 'DOCTOR' ? formatDoctorName(currentUser.name) : currentUser.name;
...
function renderDoctorsGrid(doctors) {
  ...
  doctors.forEach((doc) => {
    ...
    const docFormattedName = formatDoctorName(doc.user.name);

    const avatar = document.createElement('div');
    avatar.className = 'doc-avatar';
    avatar.textContent = doc.user.name.charAt(0) || 'D';

    const info = document.createElement('div');
    info.className = 'doc-info';

    const name = document.createElement('h3');
    name.textContent = docFormattedName;
...
function selectDoctor(doc) {
  ...
  document.getElementById('selected-doc-title').textContent = `Slots for ${docFormattedName}`;
...
function renderSlotsGrid(data) {
  ...
  if (data.isOnLeave) {
    ...
    leaveMsg.textContent = `Doctor is on leave on ${data.date}. No slots available.`;
...
function renderPatientAppointments(appointments) {
  ...
  appointments.forEach((appt) => {
    ...
    const docFormattedName = formatDoctorName(appt.doctor.user.name);

    const title = document.createElement('h3');
    title.textContent = `${docFormattedName} (${appt.doctor.specialization})`;
...
async function loadAdminDoctorsDropdown() {
  try {
    ...
    data.doctors.forEach((doc) => {
      ...
      opt.value = doc.id;
      opt.textContent = `${formatDoctorName(doc.user.name)} (${doc.specialization})`;
...
async function handleCreateDoctor(e) {
  ...
  try {
    ...
    showToast(`Doctor profile for ${formatDoctorName(name)} created successfully!`, 'success');
    loadAdminDoctorsDropdown();
    loadAdminOverview();
  } catch (err) {
    showToast(err.message, 'error');
...
async function loadAdminOverview() {
  try {
    ...
    data.appointments.forEach((appt) => {
      ...
      left.innerHTML = `<strong>${docFormattedName}</strong> (${appt.doctor.specialization}) &bull; Patient: <em>${appt.patient.name}</em><br><small style="color: var(--text-muted)">Date: ${appt.date} at ${appt.timeSlot}</small>`;
...
function testEmailDispatch() {
  showToast('Test email notification job queued successfully!', 'success');
}

function connectGoogleCalendar() {
  window.open('/api/auth/google/url?redirect=true', '_blank', 'width=600,height=700');
  showToast('Opening Google OAuth 2.0 Authorization window...', 'info');
}
